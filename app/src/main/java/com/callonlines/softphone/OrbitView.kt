...
class OrbitView @JvmOverloads constructor(
    ...
    private data class OrbitDef(
        val rxFactor: Float, val ryFactor: Float, val tilt: Float,
        val angularSpeed: Float, val phase: Float, val planetColor: Int,
        val planetRadius: Float, val glowRadius: Float
    )
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val rnd = Random(13)
        repeat(80) {
            stars += Star(rnd.nextFloat()*w, rnd.nextFloat()*h,
                0.4f + rnd.nextFloat()*1.6f, rnd.nextFloat()*6.28f)
        }
        bgPaint = Paint().apply {
            shader = RadialGradient(w*0.5f, h*0.35f, maxOf(w,h)*0.95f,
    ...
    override fun onDetachedFromWindow() { super.onDetachedFromWindow(); lastFrameNs = 0L }
}
